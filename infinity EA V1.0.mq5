...
input bool   InpRequireBodyEntry = true;     // Require body >= InpBodyRatioEntry% on fresh entry
input double InpBodyRatioEntry   = 70.0;     // Min body % of range for initial entry candle (>=)
...
//+------------------------------------------------------------------+
//| Decide whether the signal candle is a valid FRESH ENTRY candle   |
//|   When InpRequireBodyEntry is true the entry candle's body must  |
//|   be >= InpBodyRatioEntry % of its full range (same filter used  |
//|   for basket adds but applied to the initial position).          |
//+------------------------------------------------------------------+
bool IsValidEntryCandle(int dir)
  {
   // Body-ratio filter (optional)
   if(InpRequireBodyEntry)
     {
      if(BodyRatioPercent(1) < InpBodyRatioEntry)
         return(false);
     }
   // Candle color must match direction
   int candColor = CandleColor(1);
   if(dir == DIR_BUY  && candColor != CC_GREEN) return(false);
   if(dir == DIR_SELL && candColor != CC_RED)   return(false);
   return(true);
  }
...
void OnNewBar()
  {
   ...
   // First RED   candle => color1 RED   and color2 not RED
   // Optional: entry candle body must be >= InpBodyRatioEntry% (InpRequireBodyEntry)
   bool greenStart = (color1 == CC_GREEN && color2 != CC_GREEN);
   bool redStart   = (color1 == CC_RED   && color2 != CC_RED);
   if(greenStart && IsValidEntryCandle(DIR_BUY))
     {
      double sl = SLFromCandle(1, DIR_BUY);
      OpenPosition(DIR_BUY, sl);
     }
   else if(redStart && IsValidEntryCandle(DIR_SELL))
     {
